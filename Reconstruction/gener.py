'''
Data generator which also handels preprocessing and augmentation.
...
class DataGenerator(tfk.utils.Sequence):
    ...
    def __data_generation(self, ids, labels):
        ...
        for i in range(len(ids)):
            ...
            data = np.stack((labels[0][ids[i]],labels[1][ids[i]]), axis=0)

            x, affMAt = ut.affine_tr(data, 0.7, max_rot=10) # PERFORM AUGMENTATION
            if randint(0,1) == 1:
                x = ut.add_gaus_noise(x)
            x_array.append(x[0])
            y_array.append(x[1])

        x_array = np.array(x_array)
        y_array = np.array(y_array)
        x_array = np.expand_dims(x_array, axis=4)
